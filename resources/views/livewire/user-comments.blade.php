<div>
    <div class="text-sm md:items-center text-slate-600 md:flex md:justify-between">
        <div class="flex items-center gap-6 py-6">
            <a href="{{ route('user.show', $user->username) }}" class="hover:text-slate-700 ">Stories</a>
            <a href="{{ route('user.comments', $user->username) }}" class="hover:text-slate-700 font-semibold text-slate-900">Comments</a>
        </div>
    </div>
                ...
                <div class="space-y-1 flex-1">
                    <div class="flex items-center mb-2 text-xs md:text-sm justify-between text-slate-600 ">
                        <div class="flex items-center space-x-2">
                            <span>{{ $comment->created_at->isoFormat('DD MMMM YYYY') }}</span>
                        </div>
                        @if (auth()->check() AND auth()->id() == $comment->user_id)
                        <button wire:click="delete({{ $comment->id }})" onclick="confirm('Yakin mau dihapus?') || event.stopImmediatePropagation()" type="button" class="text-sm font-medium text-red-600 ">
                            <span>Delete</span>
                        </button>
                        @endif
                    </div>
                    <div class="text-sm">komentar pada <a href="{{ route('post.show', $comment->post->slug) }}" class="font-medium hover:underline leading-tight">{{ $comment->post->title }}</a></div>
                    <p class="hidden text-slate-600 md:block">{{ $comment->body }}</p>
                    <div class="flex justify-between items-center">
                        <div class="flex text-sm items-center space-x-3">
                            <div class="flex items-center">
                                <button wire:click="like({{ $comment->id }})">
                                    @if (auth()->check() AND $comment->isLikedBy(auth()->user()))
