<div>
    <div class="text-sm text-slate-600 md:flex md:justify-between">
        <div class="flex items-center gap-4">
            <span>CATEGORY</span>
            <a href="/">
                <span class="bg-slate-100 py-2 px-6 rounded-full hover:bg-slate-200">{{ $category->name }}</span>
            </a>
        </div>
        <div class="flex items-center gap-6 py-6">
            ...
            <button wire:click="$set('filter', 'Popular')"
                @class(['hover:text-slate-700', 'font-semibold text-slate-900' => $filter == 'Popular'])
            >Popular</button>

            <button wire:click="$set('filter', 'Trending')"
                @class(['hover:text-slate-700', 'font-semibold text-slate-900' => $filter == 'Trending'])
            >Trending</button>
            ...
            <li class="flex gap-6">
                ...
                <div class="space-y-1">
                    <div class="flex items-center mb-2 text-xs md:text-sm justify-between text-slate-600 ">
                        ...
                        <div x-data="{ open: false }" class="relative inline-block">
                            ...
                            <div class="bg-white ring-1 ring-black ring-opacity-5 rounded divide-y divide-gray-100">
                                <div class="space-y-1">
                                    <button x-on:click="open = false" wire:click="markAsSpam({{ $post->id }})" type="button" class="flex w-full items-center space-x-2 py-2 px-4 text-sm font-medium text-gray-600 hover:bg-gray-100 hover:text-gray-700 focus:outline-none focus:bg-gray-100 focus:text-gray-700">
                                        Mark As Spam
                                    </button>
                                ...
                                <div class="space-y-1">
                                    ...
                                    <a x-on:click="open = false" wire:click="delete({{ $post->id }})" onclick="confirm('Yakin mau dihapus?') || event.stopImmediatePropagation()" class="flex w-full items-center space-x-2 py-2 px-4 text-sm font-medium text-red-600 hover:bg-red-700 hover:text-white focus:outline-none focus:bg-red-700 focus:text-white">
                                        <span>Delete</span>
                                    </a>
                                </div>
                            </div>
                    ...
                    <div class="flex text-sm justify-between">
                        <div class="flex items-center text-slate-600 space-x-3">
